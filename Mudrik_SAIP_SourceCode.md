# Mudrik SAIP Source Export (Sanitized)

Generated (UTC): 2026-04-01T13:44:21.708Z

This is an isolated export for SAIP submission. Original project files remain unchanged.
...
## 2) Environment Structure (Values Redacted)

```dotenv
EMBEDDING_VECTOR_DIMENSIONS=REDACTED_FOR_SAIP_SECURITY
GEMINI_API_KEY=REDACTED_FOR_SAIP_SECURITY
MODEL_API_BASE=REDACTED_FOR_SAIP_SECURITY
NEXT_PUBLIC_SUPABASE_ANON_KEY=REDACTED_FOR_SAIP_SECURITY
NEXT_PUBLIC_SUPABASE_URL=REDACTED_FOR_SAIP_SECURITY
```
...
:focus-visible {
  @apply outline-none ring-2 ring-midnight/30 ring-offset-2;
}
    ...
    const { error } = await supabase.from("user_settings").upsert(
      {
        ...
        model_api_key: REDACTED_FOR_SAIP_SECURITY
        gemini_api_key: REDACTED_FOR_SAIP_SECURITY
              ...
              <table className="w-full text-start text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredRows.length === 0 ? (
                    ...
                    filteredRows.map((r) => (
                      <tr
                        key=REDACTED_FOR_SAIP_SECURITY
...
### File: `components/mudrik-logo.tsx`
...
/** Deterministic across server/client to avoid hydration mismatches from locale APIs. */
...
export type UserModelSettings = {
  ...
  modelApiKey: REDACTED_FOR_SAIP_SECURITY
  geminiApiKey: REDACTED_FOR_SAIP_SECURITY
...
export function createClient() {
  ...
  if (!url || !key) {
    ...
    throw new Error(`متغيرات Supabase العامة ناقصة: ${missing.join(", ")}`);
  }
...
### File: `scripts/export-saip-source-bundle.mjs`
...
### File: `supabase/fix_infrastructure.sql`
...
### File: `supabase/migrations/001_mudrik_schema.sql`
...
### File: `supabase/migrations/002_storage_vault.sql`
...
### File: `supabase/migrations/004_user_settings_gemini.sql`
...
### File: `tests/model-gateway.test.ts`
...
vi.mock("@google/generative-ai", () => {
  class GoogleGenerativeAI {
    ...
    constructor(apiKey: string) {
      this.apiKey = REDACTED_FOR_SAIP_SECURITY
    }
